...
export default async function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-primary/10 bg-background/60">
      <div className="max-w-7xl mx-auto px-6 md:px-10 h-24 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {/* Logo Placeholder */}
          <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center font-bold text-foreground text-lg shadow-lg shadow-primary/20">
            A
          </div>
          <Link href="/" className="text-2xl font-heading font-bold tracking-wide text-foreground">
            AuraPlan
          </Link>
        </div>
        
        <div className="hidden md:flex items-center gap-12 text-base font-medium">
          <Link href="/" className="text-foreground/80 hover:text-foreground hover:-translate-y-0.5 transition-all">
            Home
          </Link>
          <Link href="/properties" className="text-foreground/80 hover:text-foreground hover:-translate-y-0.5 transition-all">
            Properties
          </Link>
          <Link href="/#about" className="text-foreground/80 hover:text-foreground hover:-translate-y-0.5 transition-all">
            About
          </Link>
        </div>

        <div className="flex items-center gap-2 md:gap-6">
          <Link href="/host" className="hidden md:flex text-sm font-medium text-foreground hover:bg-white/5 px-5 py-2.5 rounded-full transition-colors border border-transparent hover:border-white/10 whitespace-nowrap">
            Switch to Host
          </Link>
